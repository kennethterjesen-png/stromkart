const QUERY_URL =
  'https://kart.vestmar-nett.no/arcgis/rest/services/Ekstern/StromstansPublic/MapServer/0/query?where=1%3D1&outFields=*&returnGeometry=true&f=json';

const SOURCE_URL = 'https://www.vestmar-nett.no/';
...
function normalizeFeature(feature) {
  ...
  return {
    id: `vestmar-${safeString(attr.EVENTID, 'unknown')}`,
    company: 'Vestmar Nett',
    type: normalizeType(attr.TYPE_TXT),
...
async function fetchOutages() {
  try {
    ...
    console.log(`Vestmar Nett provider: ${outages.length} hendelser etter provider-filter`);

    return outages;
  } catch (error) {
    console.error(`Vestmar Nett provider feil: ${error.message}`);
    return [];
  }
}
